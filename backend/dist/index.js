...
const schemes_1 = require("./routes/schemes");
const ai_1 = require("./routes/ai");
const csc_1 = require("./routes/csc");
const extras_1 = require("./routes/extras");
const schemeSync_1 = require("./jobs/schemeSync");
...
app.use('/csc', csc_1.cscRouter);
app.use('/status', extras_1.statusRouter);
app.use('/notes', extras_1.notesRouter);
app.use('/notifications', extras_1.notificationsRouter);
...
app.post('/internal/sync-now', async (req, res) => {
    ...
    try {
        const result = await (0, schemeSync_1.runSchemeSync)('manual');
...
app.get('/health', async (_req, res) => {
    const database = await (0, db_1.checkDatabaseReady)();
    if (!database.ok) {
...
async function seedCSCCentres() {
    try {
        const { count } = await db_1.appDb.from('csc_centres').select('*', { count: 'exact', head: true });
        if (!count || count === 0) {
            console.log('Seeding 50 CSC centres...');
            const { error } = await db_1.appDb.from('csc_centres').insert(CSC_SEED_DATA);
            if (error)
                console.error('CSC seed error:', error.message);
            else
                console.log('Seeded 50 CSC centres');
        }
        else {
...
async function startup() {
    try {
        ...
        if (shouldRunStartupTasks) {
            try {
                const { count } = await db_1.appDb.from('schemes').select('*', { count: 'exact', head: true });
                if (!count || count === 0) {
                    console.log('Schemes table empty — triggering initial sync...');
                    (0, schemeSync_1.runSchemeSync)('initial').catch(console.error);
                }
            }
            catch (e) {
