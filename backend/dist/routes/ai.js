...
const SUPPORTED_TRANSCRIPTION_TYPES = {
    'audio/flac': 'flac',
    'audio/mp3': 'mp3',
    'audio/mp4': 'mp4',
    'audio/mpeg': 'mpeg',
...
async function fetchProfile(authId) {
    const { data } = await db_1.appDb.from('user_scheme_profile_inputs').select('*').eq('auth_id', authId).single();
    return data;
}
...
function scoreTextRelevance(scheme, queryTokens) {
...
async function rankSchemes(profile, message = '', limit = 8) {
    ...
    return schemes
        .map((scheme) => ({
        ...scheme,
        eligibility: profile ? (0, eligibility_1.scoreScheme)(profile, scheme.eligibility_criteria || {}) : { score: 0 },
        relevanceScore: scoreTextRelevance(scheme, queryTokens)
    }))
        .filter((scheme) => {
        if (!queryTokens.length)
            return scheme.eligibility.score >= 65;
...
async function fetchPersonalizedSchemes(profile, limit = 8) {
    return rankSchemes(profile, '', limit);
}
function buildSchemeContext(schemes) {
    if (!schemes.length)
        return 'No strongly eligible schemes found for this profile.';
...
function buildSystemPrompt(user, lang, schemeContext = '') {
...
function schemeLine(scheme, index) {
...
async function buildFallbackReply(message, profile, lang) {
...
exports.aiRouter.post('/transcribe', async (req, res) => {
    try {
        if (!groq) {
        ...
        const result = await groq.audio.transcriptions.create({
            file: await (0, groq_sdk_1.toFile)(audio, `voice.${extension}`, { type: normalizedMimeType }),
...
exports.aiRouter.post('/form-assist', async (req, res) => {
    try {
        const { form_field_label, form_context, language = 'ta' } = req.body;
        const { data: user } = await db_1.appDb.from('user_registration_profiles').select('*').eq('auth_id', res.locals.user.id).single();
