...
exports.getAppUserByAuthId = getAppUserByAuthId;
...
async function replaceRows(table, userId, rows) {
    await db_1.appDb.from(table).delete().eq('user_id', userId);
    if (rows.length > 0) {
        const { error } = await db_1.appDb.from(table).insert(rows);
        if (error)
            throw error;
    }
}
...
async function saveProfileData(userId, data) {
    const { error: profileError } = await db_1.appDb.from('user_profiles').upsert({
        user_id: userId,
    ...
    if (data.minority_status) {
        const { error } = await db_1.appDb.from('user_minority_details').upsert({
            user_id: userId,
        ...
        const { error } = await db_1.appDb.from('user_minority_details').delete().eq('user_id', userId);
        if (error)
            throw error;
    }
    if (data.is_disabled) {
        const { error } = await db_1.appDb.from('user_disabilities').upsert({
            user_id: userId,
        ...
        const { error } = await db_1.appDb.from('user_disabilities').delete().eq('user_id', userId);
        if (error)
            throw error;
    }
    const { error: economicError } = await db_1.appDb.from('user_economic_profiles').upsert({
        user_id: userId,
    ...
    if (economicError)
        throw economicError;
    await replaceRows('user_existing_scheme_benefits', userId, data.existing_scheme_benefits
        ? csvNames(data.existing_scheme_names).map(scheme_name => ({ user_id: userId, scheme_name }))
        : []);
    const { error: educationError } = await db_1.appDb.from('user_education_profiles').upsert({
        user_id: userId,
    ...
    const { error: occupationError } = await db_1.appDb.from('user_occupation_profiles').upsert({
        user_id: userId,
    ...
    if (hasBusiness) {
        const { error } = await db_1.appDb.from('user_business_profiles').upsert({
            user_id: userId,
        ...
        const { error } = await db_1.appDb.from('user_business_profiles').delete().eq('user_id', userId);
        if (error)
            throw error;
    }
    const { error: householdError } = await db_1.appDb.from('user_household_profiles').upsert({
        user_id: userId,
    ...
    await replaceRows('user_special_statuses', userId, Object.entries(SPECIAL_STATUS_FIELDS)
        .filter(([field]) => !!data[field])
        .map(([, status_code]) => ({ user_id: userId, status_code })));
    await replaceRows('user_document_possessions', userId, Object.entries(DOCUMENT_FIELDS)
        .filter(([field]) => !!data[field])
        .map(([, document_type]) => ({ user_id: userId, document_type })));
}
async function completeRegistration(userId, lockedUntil) {
