...
exports.scoreScheme = scoreScheme;
...
function normalizeCriteria(criteria) {
    ...
    if (normalized.is_migrant_worker === true) {
    ...
    if (normalized.is_bpl === true && normalized.is_bpl_required == null) {
        normalized.is_bpl_required = true;
    }
    const possession = new Set(normalized.possession_required || []);
    if (normalized.has_aadhaar === true)
        possession.add("aadhaar");
    if (normalized.has_jan_dhan === true)
        possession.add("jan_dhan");
    if (normalized.has_voter_id === true)
        possession.add("voter_id");
    if (possession.size > 0)
        normalized.possession_required = Array.from(possession);
    return normalized;
}
function capScore(score, cap) {
    return Math.min(score, cap);
}
function scoreScheme(rawProfile, rawCriteria) {
    ...
    if (criteria.is_bpl_required == null && (!criteria.ration_card_type || criteria.ration_card_type.length === 0)) {
        ...
        let bplMatch = true;
        if (criteria.is_bpl_required === true && profile.is_bpl !== true)
            bplMatch = false;
    ...
    if (criteria.is_disabled_required == null && criteria.disability_percent_min == null) {
        ...
        if (criteria.is_disabled_required === true && profile.is_disabled !== true)
    ...
    if (criteria.land_min_acres == null && criteria.land_max_acres == null) {
        ...
        if (landMatch && criteria.land_min_acres != null && land < criteria.land_min_acres)
            landMatch = false;
        if (landMatch && criteria.land_max_acres != null && land > criteria.land_max_acres)
            landMatch = false;
        if (landMatch)
            score += 4;
        else {
    ...
    if (!criteria.special_status_required || criteria.special_status_required.length === 0) {
        ...
        if (criteria.special_status_required.includes("widow") && profile.is_widow)
            specialMatch = true;
        if (criteria.special_status_required.includes("armed_forces") && profile.is_armed_forces_family)
            specialMatch = true;
        if (criteria.special_status_required.includes("hiv_affected") && profile.is_hiv_affected)
            specialMatch = true;
        if ((criteria.special_status_required.includes("migrant") || criteria.special_status_required.includes("migrant_worker")) && profile.is_migrant)
            specialMatch = true;
        if (criteria.special_status_required.includes("fisherman") && profile.is_fisherman)
            specialMatch = true;
        if (criteria.special_status_required.includes("weaver") && profile.is_weaver)
            specialMatch = true;
        if (criteria.special_status_required.includes("pvtg") && profile.is_pvtg)
            specialMatch = true;
        if (criteria.special_status_required.includes("orphan_destitute") && profile.is_orphan_destitute)
            specialMatch = true;
        if (criteria.special_status_required.includes("chronic_illness_family") && profile.has_chronic_illness_family)
            specialMatch = true;
        if (criteria.special_status_required.includes("covid_affected") && profile.covid_affected)
            specialMatch = true;
        if (specialMatch)
            score += 7;
        else {
    ...
    if (!criteria.possession_required || criteria.possession_required.length === 0) {
        ...
        if (criteria.possession_required.includes("aadhaar") && !profile.has_aadhaar)
            possessionMatch = false;
        if ((criteria.possession_required.includes("jan_dhan") || criteria.possession_required.includes("aadhaar_linked_bank_account")) && !profile.has_jan_dhan)
            possessionMatch = false;
        if (criteria.possession_required.includes("voter_id") && !profile.has_voter_id)
            possessionMatch = false;
        if (possessionMatch)
            score += 3;
        else {
    ...
    if (hardMismatch)
        score = capScore(score, 49);
    else if (missingRequired)
        score = capScore(score, 64);
