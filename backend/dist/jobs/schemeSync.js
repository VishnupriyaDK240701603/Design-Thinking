...
exports.runSchemeSync = runSchemeSync;
...
function documentNotes(value) {
    if (typeof value === 'string')
        return value;
    return value?.name_ta || value?.name_en || value?.name || value?.document_type || value?.type || 'Other';
}
async function replaceSchemeDocuments(schemeId, documents = []) {
    await db_1.appDb.from('scheme_required_documents').delete().eq('scheme_id', schemeId);
    ...
    if (rows.length > 0) {
        await db_1.appDb.from('scheme_required_documents').upsert(rows, { onConflict: 'scheme_id,document_type' });
    }
}
async function replaceSchemeSteps(schemeId, steps = []) {
    await db_1.appDb.from('scheme_application_steps').delete().eq('scheme_id', schemeId);
    ...
    if (rows.length > 0) {
        await db_1.appDb.from('scheme_application_steps').insert(rows);
    }
}
async function runSchemeSync(trigger) {
    ...
    try {
        ...
        let govtSchemes = [];
        try {
            const response = await axios_1.default.get(`${GOVT_API_URL}/api/schemes?limit=200`);
        ...
        for (const gs of govtSchemes) {
            ...
            const { data: existing } = await db_1.appDb.from('schemes').select('id').eq('government_scheme_id', gs.id).single();
            if (existing) {
                await db_1.appDb.from('schemes').update(schemeData).eq('id', existing.id);
                await replaceSchemeDocuments(existing.id, gs.documents_required || []);
                await replaceSchemeSteps(existing.id, gs.application_steps || []);
                schemesUpdated++;
            }
            else {
                const { data: newScheme } = await db_1.appDb.from('schemes').insert(schemeData).select().single();
                schemesAdded++;
                ...
                if (newScheme) {
                    ...
                    const { data: users } = await db_1.appDb.from('user_scheme_profile_inputs').select('*');
                    if (users) {
                        ...
                        for (const user of users) {
                            ...
                            // Score main user
                            const elig = (0, eligibility_1.scoreScheme)(user, newScheme.eligibility_rules || {});
                            if (elig.score >= 65 && userNotifCounts[userId] < 3) {
                                await db_1.appDb.from('notifications').insert({
                                    user_id: userId,
                                    type: 'scheme_match',
        ...
        if (fetchedGovtIds.length > 0) {
            const { data: deactivated } = await db_1.appDb
                .from('schemes')
        ...
        const { data: expiringSchemes } = await db_1.appDb
            .from('schemes')
            .select('*')
            .eq('is_active', true)
        ...
        if (expiringSchemes && expiringSchemes.length > 0) {
            const { data: allUsers } = await db_1.appDb.from('user_scheme_profile_inputs').select('id');
            if (allUsers) {
        ...
        await db_1.appDb.from('sync_logs').insert({
            source: 'govt-api',
            entity: 'schemes',
            status: 'success',
...
node_cron_1.default.schedule('0 0 * * *', () => runSchemeSync('cron'), { timezone: 'Asia/Kolkata' });
